    ...
    <div className="min-h-screen w-full bg-zinc-900 flex justify-center p-4">
      <div className="w-full max-w-md bg-zinc-800/50 backdrop-blur-xl rounded-xl border border-zinc-700/50 shadow-xl p-8 space-y-8">
        ...
        <Button
          ...
          className={`w-full bg-gradient-to-r transition-all duration-500 relative overflow-hidden ${
            results.length > 0
              ? "from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 scale-105 animate-rainbow-border"
              : "from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 scale-105 animate-rainbow-border"
          } text-white shadow-lg shadow-indigo-500/25 border-0 before:absolute before:inset-0 before:bg-gradient-to-r before:from-pink-500 before:via-purple-500 before:to-indigo-500 before:animate-rainbow-flow before:opacity-0 ${
            results.length > 0 ? "before:opacity-30" : ""
          }`}
        >
          计算
        </Button>
        ...
        {results.length > 0 && (
          <div className="space-y-4">
            ...
            <div className="bg-zinc-800/50 rounded-lg border border-zinc-700/50 p-8 max-h-[400px] overflow-y-auto space-y-4">
              <ul className="space-y-2 list-disc list-inside">
                {results.map((result, index) => (
                  <ol key={index} className="text-zinc-300 pl-4 indent-[-1em] font-mono">
                    {result}
                  </ol>
                ))}
              </ul>
